...
- Where the repository keeps a note of the designs that look like faults and are not, read it before calling one of them a fault, and never undo one of them without asking. A session meeting a deliberate design cold will read it as a bug, and the note is what stops it being "fixed" every few months.
