...
e1.5.7 and the MCMv3 API, last updated February 2021; what it gives is still vanilla rumour data
with vanilla's staleness. TradeLord reads every market live, or only what you have seen in person.
...
goods are covered only by vanilla's own flags. TradeLord is one module, one dependency.
...
distance limit at all; and it only touches trade goods, so loot, livestock and animals are outside
it.
...
TradeLord will not do it, because it would change the economy for everyone else. 10369 predicts what
each caravan will buy on arrival, which TradeLord does not: we count the cargo they already carry.
11988 records what the screen said rather than modelling it. 11607 fits an automation family you may
already run.
