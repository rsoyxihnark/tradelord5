...
SHARED_PASS_RULES = {
    ...
    "WhatTheMarkKeeps(": ('Passes.cs', "internal static ForTheMark[] WhatTheMarkKeeps"),
}
...
def the_marker_counts_only_what_the_selling_rules_would_really_move():
    ...
    return ("cargo.Add((el.EquipmentElement, el.Amount - keep, TradePolicy.WorthToBeat(el.EquipmentElement)));" in carried
            ...
            and "HoldCargoForBestMarket" not in S['Marker.cs']
            and "markId != null && markId != hereId &&" in S['Rules.cs']
...
def readme_defaults_match_the_shipped_ones():
    ...
    hold = str(round(float(option_default('HoldCargoForBestMarket').rstrip('f')) * 100))
    claims = ['which ships at ' + share + '% so one cheap good cannot take your whole cargo',
              'sells it on the way only for at least ' + hold + '% of what that market pays',
...
chk("1.3.16", "the hold-for-best-market floor is tested against every unit as the lot drains, and a test holds it to that",
    ...
    and "floor > 0 && boughtLeft > 0 && boughtLeft <= holdFor && BelowTheBestMarket(price, floor);" in S['Rules.cs']
    ...
    and all(one in FLOORTESTS for one in
            ("The_floor_is_the_other_market_price_less_the_tolerance_you_set",
             ...
             "Only_the_units_the_marked_market_would_buy_are_held_and_only_below_the_floor",
             "The_floor_for_the_units_left_is_your_share_of_what_the_mark_pays_for_the_last_of_them",
             "new Random(5540)")))
...
chk("1.21.0", "the sell-side floor is Hold cargo for the best market and nothing else, so it binds every unit you bought against what the marked market pays for it, up to as many as that market would buy, or none",
    ...
    "int holdFor = holding?[at].Units ?? 0;" in S['Passes.cs'] and
...
chk("1.21.0", "loot goes to the first market that can pay, since nothing but Hold cargo for the best market raises the floor and it never holds a unit you did not buy",
    ...
    "if (!basis.SetTheBoughtUnitsAside(ref remaining)) break;" in sell_pass() and
    "Loot_is_never_held_for_the_marked_market" in SELLPASSTESTS and
    "In_a_stack_of_bought_and_looted_units_the_bought_ones_are_held_and_the_loot_goes" in SELLPASSTESTS and
    S['Passes.cs'].count("s.HoldCargoForBestMarket") == 3)
...
def the_hold_hint_says_what_the_share_holds_and_when_it_holds_nothing():
    en = spoken(ENGLISH)
    hold = en.get('TL329', '')
    ...
    return ("On the way, sell a good you bought only for at least this share of what the market" in hold
            ...
            and "{=TL330}" not in M
            and hold in M
            and all(one and one != hold for one in said))
...
chk("1.21.0", "the hold hint says the share holds back only goods you bought, only as many as the marked market would buy, and nothing at 0%, in the marked market or while that marker is off",
    the_hold_hint_says_what_the_share_holds_and_when_it_holds_nothing())
...
def no_setting_a_player_ever_saved_is_left_stranded():
    ...
    return not stranded and not unlisted and len(EVER_SHIPPED) >= 88
...
def every_best_market_comparison_prices_the_quality_you_carry():
    ...
    resale = method_body(S['Trading.cs'], "public bool ResaleMarket(int at, int units, int worth, out int[] rungs)")
    takes = method_body(S['Marker.cs'], "internal static int[] WhatTheMarkTakes")
    step = method_body(S['Marker.cs'], "private int Next()")
    ...
    return ("long priced = (long)plainPrice * qualityValue / plainValue;" in scale
            and "rungs = Marker.WhatTheMarkTakes(_mark, _plan[at].EquipmentElement, units, worth," in resale
            and "Paying pays = WhatThatMarketPays(mark, mark.SettlementComponent, el, party, ride);" in takes
...
chk("1.91.1", "Hold cargo for the best market, the map marker and Color prices by world market hold a good of any quality to what the market they weigh pays for that same quality",
...
def a_haul_animal_is_bought_only_for_what_a_full_cargo_left_behind_and_the_gold_left_can_fill():
    ...
    return (haul and buy and each and carries and wanted
            and "private static (float weight, int cost, float profit, bool food) _unfitted;" in t
...
def what_a_full_cargo_left_behind_is_counted_at_the_price_it_would_climb_to_goods_and_food_alike():
    t = S['Trading.cs']
    left = method_body(S['Passes.cs'], "private static (int units, int cost, float profit) UnitsTheHoldLeftBehind")
    ...
    return (left and them and ahead and supply and food
            ...
            and ordered(left, "Func<int, int> ahead = market.PricesAhead(at);", "int price = ahead(units);",
                        "if (price <= 0) break;", "cost = TradeMath.AddedUp(cost, price);", "return (units, cost, profit);")
...
def no_haul_animal_is_bought_for_goods_that_would_fill_less_than_half_of_it():
    ...
    return (haul and wanted
            ...
            and "float leastFilled = Herding.LeastFilledFor(unfitted.food," in haul
            and ordered(haul, "roomLeft, stillToCarry, leastFilled,",
                        "tooLittle = hauled == 0 &&",
                        "roomLeft, stillToCarry);\n                            enough = true;",
                        "if (hauled <= 0)", "else if (tooLittle)",
            ...
            and all(one in HERDTESTS for one in
                    ("No_haul_animal_is_bought_for_goods_that_would_fill_less_than_half_of_it",
                     "A_haul_animal_asked_to_be_filled_by_nothing_in_particular_is_bought_however_little_it_carries"))
            and "It buys no haul animal for goods that would fill less than half of one" in README)


chk("1.93.7", "a haul animal for goods is weighed on how much of it the goods the gold left can buy would fill beyond the room already there, half of it being enough, and TradeLord.log says when the goods were too few",
    no_haul_animal_is_bought_for_goods_that_would_fill_less_than_half_of_it())
...
def a_haul_animal_is_never_bought_if_it_would_take_the_purse_down_to_its_floor():
    ...
    return (haul
            ...
            and ordered(haul, "if (hauled <= 0)", "if (floored)",
                        ...
                        "else if (tooLittle)")
            and said in spoken(ENGLISH)['TL426']
            and said in M
            and said_in_every_language('TL426')
            and "\u00f6dendikten sonra" in spoken(TRANSLATIONS['T\u00fcrk\u00e7e'])['TL426']
            and "\u043f\u043e\u0441\u043b\u0435 \u043e\u043f\u043b\u0430\u0442\u044b" in spoken(TRANSLATIONS['\u0420\u0443\u0441\u0441\u043a\u0438\u0439'])['TL426']
            and "\u4ed8\u5b8c\u9a6e\u517d\u7684\u94b1\u540e" in spoken(TRANSLATIONS['\u7b80\u4f53\u4e2d\u6587'])['TL426']
            and "A_haul_animal_is_bought_only_while_your_purse_stays_above_the_floor_once_it_is_paid_for" in HERDTESTS
            and "It never buys one that would leave your purse at 2000 denars or less" in README)
...
def light_goods_that_would_pay_for_a_haul_animal_still_get_one():
    haul = method_body(S['Trading.cs'], "public static bool ExecuteHaulage")
    wanted = method_body(S['Rules.cs'], "internal static bool AnotherHaulAnimalIsWanted")
    left = method_body(S['Passes.cs'], "private static (int units, int cost, float profit) UnitsTheHoldLeftBehind")
    them = method_body(S['Passes.cs'], "internal static Traded BuyThem")
    return (haul and wanted and left and them
            and ordered(wanted, "float profitToCarry = 0f, int price = 0)",
                        "if (beyond >= (double)each * (least > 0f ? least : 0f)) return true;",
            ...
            and "moved.UnfittedProfit += left.profit;" in them
            ...
            and "unless those goods would make more than the animal costs" in README)


chk("1.93.8", "goods that would fill less than half of a haul animal still get one when the profit of the part it would carry is at least what the animal costs, the profit counted on the same units, prices and margin as the goods left behind",
    light_goods_that_would_pay_for_a_haul_animal_still_get_one())


def food_gets_a_haul_animal_it_would_fill_less_than_half_of_only_on_its_last_day():
    haul = method_body(S['Trading.cs'], "public static bool ExecuteHaulage")
    return (haul
            ...
            and "Food_gets_a_haul_animal_it_would_fill_less_than_half_of_only_once_the_party_is_down_to_its_last_day"
                in HERDTESTS
            and "It buys one for food only when the food would fill at least half of it, or when your party is down to its last day of food"
                in README)


chk("1.93.8", "food a full cargo left behind gets a haul animal it would fill less than half of only once the food your party carries is down to its last day, and TradeLord.log says when the food was too little",
    food_gets_a_haul_animal_it_would_fill_less_than_half_of_only_on_its_last_day())


def a_dry_run_takes_away_the_room_of_the_animals_it_would_sell():
    held = method_body(S['Trading.cs'], "private sealed class Pass")
    sold = method_body(S['Trading.cs'], "private float CapacitySoldOnPaper")
    mount = method_body(S['Drove.cs'], "internal static float CargoASpareMountAdds")
    rule = method_body(S['Rules.cs'], "internal static float CargoASpareMountAdds")
    return (held and sold and mount and rule
            and ("(_capacity < 0f ? _capacity = Carry.Capacity(Party) : _capacity) + Books.CapacityAdded(Sim) -\n"
                 "                CapacitySoldOnPaper();") in held
            and ordered(sold, "int hauls = Books.HaulsShed(Sim), mounts = Books.MountsShed(Sim);",
                        ...
                        "sold += mounts * _eachMount;", "return sold;")
            and ordered(mount, "if (party == null || Carry.Sailing()) return 0f;",
            ...
            and "internal const int ItemsASpareMountCarries = 2;" in S['Rules.cs']
            and ordered(rule, "float each = (float)ItemsASpareMountCarries * averageWeight;",
                        "if (addedUp > 0f && capacity > addedUp) each *= capacity / addedUp;")
            and "internal int HaulsShed(bool sim) => OnPaper(sim) ? _hauls : 0;" in S['Books.cs']
            and "internal int MountsShed(bool sim) => OnPaper(sim) ? _mounts : 0;" in S['Books.cs']
            and "A_spare_mount_carries_what_the_game_gives_one_and_its_share_of_a_bonus_on_the_whole_hold" in HERDTESTS
            and "The dry run also takes away the cargo room of the haul animals and spare mounts it would sell" in README)
...
def hold_cargo_and_the_marker_floor_count_what_is_on_its_way():
    resale = method_body(S['Trading.cs'], "public bool ResaleMarket(int at, int units, int worth, out int[] rungs)")
    mark = method_body(S['Trading.cs'], "private Settlement TheMark()")
    takes = method_body(S['Marker.cs'], "internal static int[] WhatTheMarkTakes")
    en = spoken(ENGLISH)
    tr, ru, cn = (spoken(TRANSLATIONS[k]) for k in TRANSLATIONS)
    return ("BestSellAsItLands" not in ALL
            and "float ride = Travel.EstimateDaysFromParty(mark);" in mark
            and "_markRide, _markPurse);" in resale
...
def hold_cargo_is_one_share_on_at_three_quarters():
    now = settings_now()
    en = spoken(ENGLISH)
    return (option_default('HoldCargoForBestMarket') == '0.75f'
            and now.get('HoldCargoForBestMarket') == 'float'
            and 'PreferBestSellTown' not in now and 'BestSellTownTolerance' not in now
            and '[SettingPropertyFloatingInteger("{=TL229}Hold cargo for the best market", 0f, 1f, "#0%", Order = 6,' in M
            and "public float HoldCargoForBestMarket { get => _o.HoldCargoForBestMarket; "
                "set { _o.HoldCargoForBestMarket = value; Options.Bump(); } }" in M
            and "PreferBestSellTown" not in M and "BestSellTownTolerance" not in M and "{=TL230}" not in M
            and '{ "HoldCargoForBestMarket", new double[] { 0, 1 } },' in S['Migrate.cs']
            and '"BestSellTownTolerance", new double[]' not in S['Migrate.cs']
            and en.get('TL229') == "Hold cargo for the best market"
            and all('TL230' not in spoken(path) for path in [ENGLISH] + list(TRANSLATIONS.values()))
            and "Three_quarters_of_what_the_marked_market_pays_is_where_the_hold_lets_go" in SELLPASSTESTS
            and "A_share_of_nothing_holds_nothing" in SELLPASSTESTS
            and "TheShareIsHeldBetweenNothingAndAll" in MIGRATIONTESTS)

chk("1.94.0", "Hold cargo for the best market is one slider from 0% to 100% under the name it always had, 75% out of the box, with no switch left beside it",
    hold_cargo_is_one_share_on_at_three_quarters())


def an_old_hold_setting_is_dropped_and_the_share_starts_at_what_tradelord_ships_with():
    lift = method_body(S['Migrate.cs'], "public static bool Lift")
    step = method_body(S['Migrate.cs'], "private static bool HoldCargoBecameOneShare")
    return (ordered(lift, "if (from < 15) changed |= WhatToBuyFirstIsOneRuleNow(written, notes);",
                    "if (from < 18) changed |= HoldCargoBecameOneShare(written, notes);")
            and 'foreach (string was in new[] { "PreferBestSellTown", "BestSellTownTolerance" })' in step
            and ordered(step, "if (!written.TryGetValue(was, out string held)) continue;",
                        "written.Remove(was);", "changed = true;",
                        "notes?.Add(", "new Options().HoldCargoForBestMarket * 100f")
            and "written[" not in step
            and "HoldCargoForTheBestMarketIsOneShareThatStartsAtWhatTradeLordShipsWith" in MIGRATIONTESTS
            and "AFileAlreadyInTheOneShareShapeIsLeftAlone" in MIGRATIONTESTS)

chk("1.94.0", "a settings file that still carries the old Hold cargo switch or its tolerance drops both, writes a note naming each, and the share starts at what TradeLord ships with",
    an_old_hold_setting_is_dropped_and_the_share_starts_at_what_tradelord_ships_with())


def the_hold_is_asked_only_of_the_mark_and_never_inside_it():
    mark = method_body(S['Marker.cs'], "internal static Settlement TheMarkToHoldFor")
    walk = method_body(S['Marker.cs'], "internal static void NoteTheWalkIn")
    load = method_body(S['Marker.cs'], "internal static void NoteALoadInside")
    forget = method_body(S['Marker.cs'], "internal static void Forget()")
    entered = method_body(S['Trading.cs'], "private void OnSettlementEntered")
    launched = method_body(S['Trading.cs'], "private void OnSessionLaunched")
    resale = method_body(S['Trading.cs'], "public bool ResaleMarket(int at, int units, int worth, out int[] rungs)")
    themark = method_body(S['Trading.cs'], "private Settlement TheMark()")
    holds = between(S['Rules.cs'], "internal static bool HoldsFor(string markId, string hereId, string walkedIntoId, string markedOnTheWayInId) =>", ";")
    return (ordered(mark, "Settlement mark = _picked;",
                    ...
                    "return mark;")
            and "markId != null && markId != hereId &&" in holds
            and "!(hereId != null && hereId == walkedIntoId && markedOnTheWayInId == hereId)" in holds
            and ordered(walk, "_walkedInto = at?.StringId;", "_markedOnTheWayIn = _picked?.StringId;")
            and ordered(load, "_walkedInto = at?.StringId;", "_markedOnTheWayIn = at?.StringId;")
            and ordered(forget, "_walkedInto = null;", "_markedOnTheWayIn = null;")
            and ordered(entered, "if (party != MobileParty.MainParty) return;",
                        "if (!StillTheSameArrival(settlement)) Marker.NoteTheWalkIn(settlement);",
                        "Counter.Forget();")
            and ordered(launched, "Marker.ForgetTheRead();",
                        "if (inside != null) Marker.NoteALoadInside(inside);",
                        'Guard.Run("Action.RestoreMarker", Marker.Update);')
            and S['Trading.cs'].count("Marker.NoteTheWalkIn(") == 1
            and S['Trading.cs'].count("Marker.NoteALoadInside(") == 1
            and ordered(themark, "if (!_markRead)", "_markRead = true;",
                        "Settlement mark = Marker.TheMarkToHoldFor(_pass.Site);",
                        "if (!TradeMath.OutOfReach(ride) && purse > 0)",
                        "return _mark;")
            and ordered(resale, "if (TheMark() == null) return false;", "return rungs.Length > 0;")
            and "public int ResalePurse() => TheMark() == null ? 0 : _markPurse;" in S['Trading.cs']
            and all(one in SCORINGTESTS for one in
                    ("Cargo_is_held_for_a_marked_market_you_are_not_standing_in",
...
def the_hold_counts_only_what_the_mark_would_buy_of_what_you_bought():
    sell = method_body(S['Passes.cs'], "internal static Traded SellThem")
    ...
    return (ordered(sell, "ForTheMark[] holding = s.HoldCargoForBestMarket > 0f ? WhatTheMarkKeeps(market, books, sim, s) : null;",
            ...
            and ordered(takes, "if (rungAt == null || carried <= 0) return new int[0];",
                        ...
                        "fetched += price;", "taken[count++] = price;",
                        "Array.Resize(ref taken, count);")
            and "bool ResaleMarket(int at, int units, int worth, out int[] rungs);" in S['Passes.cs']
            and all(one in MATHTESTS for one in
                    ("The_marked_market_takes_the_units_that_clear_your_margin_at_what_it_pays_for_each",
                     "The_marked_market_takes_no_more_than_its_purse_or_than_you_carry",
                     "A_marked_market_that_pays_nothing_or_misses_your_margin_takes_nothing"))
            and all(one in SELLPASSTESTS for one in
                    ("Only_as_many_as_the_marked_market_would_buy_are_held",
                     "Units_kept_back_from_a_bought_stack_never_let_the_rest_go_under_the_floor",
                     "The_marked_market_is_asked_about_the_bought_units_at_the_margin_they_are_sold_against",
                     "Your_margin_still_binds_where_the_hold_would_let_a_sale_through",
                     "A_unit_that_misses_both_is_named_as_missing_your_margin")))

chk("1.94.0", "Hold cargo for the best market weighs a good against what the marked market pays for the units you bought that it would take within your margin and its gold, holds only that many, and never stands in for your margin",
    the_hold_counts_only_what_the_mark_would_buy_of_what_you_bought())


def the_hold_is_named_and_described_as_it_now_works():
    en = spoken(ENGLISH)
    return (en.get('TL85') == "Hold cargo for the best market keeps your cargo for the marked market"
            ...
            and "once you switch it on" not in README
            and "BestSellAsItLands" not in ALL)

chk("1.94.0", "the reason given when cargo is held names Hold cargo for the best market and the marked market in every language, and the feature list says what the share holds",
    the_hold_is_named_and_described_as_it_now_works())
...
def the_marked_market_s_gold_is_shared_across_every_good_you_carry():
    ...
    return (ordered(keeps, "int till = TradeRules.WhatTheTillCanPay(sim ? market.Till() : market.TillNow(), market.Village);",
            ...
            and ordered(share, "if (purse <= 0 || share <= 0f) return kept;",
                        ...
                        "left -= rungs[g][u];", "kept[g]++;")
            and "public int ResalePurse() => TheMark() == null ? 0 : _markPurse;" in S['Trading.cs']
            and "int ResalePurse();" in S['Passes.cs']
            and all(one in FLOORTESTS for one in
                    ("The_purse_of_the_marked_market_goes_to_the_units_this_market_pays_least_for_against_it",
...
def each_unit_is_weighed_against_what_the_mark_pays_for_the_last_one_it_keeps():
    return ("internal static int FloorForTheLast(int[] rungs, int boughtLeft, float share) =>" in S['Rules.cs']
            and "rungs == null || boughtLeft <= 0 || boughtLeft > rungs.Length" in S['Rules.cs']
            and "? 0 : BestMarketFloor(rungs[boughtLeft - 1], share);" in S['Rules.cs']
            and "int floor = TradeRules.FloorForTheLast(there, boughtLeft, s.HoldCargoForBestMarket);" in S['Passes.cs']
            and "bestMarketFloor" not in S['Passes.cs'] and "floorKnown" not in S['Passes.cs']
            and "average" not in method_body(S['TradeMath.cs'], "public static int[] WhatTheMarkTakes")
            and all(one in SELLPASSTESTS for one in
                    ("The_units_the_marked_market_pays_most_for_are_held_and_the_rest_sell_here",
                     "The_units_the_marked_market_pays_little_for_sell_here_even_when_its_average_is_high",
                     "A_price_that_falls_as_the_stack_sells_is_held_once_it_drops_under_your_share"))
            and "The_floor_for_the_units_left_is_your_share_of_what_the_mark_pays_for_the_last_of_them" in FLOORTESTS)

chk("1.94.1", "Hold cargo for the best market weighs each unit you bought against your share of what the marked market pays for the last unit it would keep, so the units it pays most for are held and the ones it pays little for sell here",
    each_unit_is_weighed_against_what_the_mark_pays_for_the_last_one_it_keeps())


def the_look_ahead_leaves_the_food_reserve_and_quest_goods_to_the_sale():
    t = S['Trading.cs']
    body = method_body(t, "public int HoldableUnits(int at)")
    return (ordered(body, "if (!_aheadCopied)", "_aheadCopied = true;",
            ...
            and "_keepBack," not in body and "_awaited," not in body
            and t.count("_keepBackAhead") == 3 and t.count("_awaitedAhead") == 3
            and "TradePolicy.MaySell(good, _plan[at], _pass.Locked, _keepBack, _awaited," in t
            and "int HoldableUnits(int at);" in S['Passes.cs'])

chk("1.94.1", "the look ahead that shares out the gold of the marked market reads the food reserve and the goods a quest waits on from copies, so the sale that follows still keeps every one of them",
    the_look_ahead_leaves_the_food_reserve_and_quest_goods_to_the_sale())


def the_log_names_what_the_hold_kept():
    t = S['Trading.cs']
    sell = method_body(t, "private static void SellPass")
    held = method_body(t, "public void HeldBack(int at, int units, int there, int here)")
    said = method_body(t, "internal List<string> HeldFor()")
    return ("internal readonly Dictionary<ItemObject, (string where, int units, int there, int here)> Held =" in t
            and ordered(held, "if (item == null || _mark == null || units <= 0) return;",
                        "if (_pass.Held.TryGetValue(item, out var was))",
                        "units += was.units;", "there = Math.Min(there, was.there);",
            ...
            and 'if (tally.Any) Log.Repeatable(label + "-empty " + pass.Key, tally.Summary() + held,' in sell
            and 'label + " moved nothing " + pass.Where + ": " + tally.Summary() + held);' in sell
            and "void HeldBack(int at, int units, int there, int here);" in S['Passes.cs']
            and ordered(method_body(S['Passes.cs'], "internal static Traded SellThem"),
                        "tally.Note(Block.BelowBestMarket);",
                        "market.HeldBack(at, boughtLeft, there[boughtLeft - 1], price);",
                        "if (!basis.SetTheBoughtUnitsAside(ref remaining)) break;")
            and 'Assert.Equal(("iron", 5, 220, 160), market.Held[0]);' in SELLPASSTESTS)

chk("1.94.1", "TradeLord.log names each good Hold cargo for the best market kept, how many, for which market and at what price, after a pass that sold and inside the one line a pass that sold nothing writes, keyed as before so a changed hold is never hidden",
    the_log_names_what_the_hold_kept())
